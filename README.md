...
1. Create your development project.
   a) Access hkube artifacts.
     ...
     * io.hkube:java-algo-parent:1.3.01:pom
     * io.hkube:interfaces:1.3.01:jar
   ...
   b) No access to the internet
     ...
     run:
     mvn dependency:get -Dartifact=io.hkube:java-algo-parent:1.3.01:pom -DremoteRepositories=https://oss.sonatype.org/content/repositories/snapshots -Ddest=java-algo-parent.xml
     mvn dependency:get -Dartifact=io.hkube:interfaces:1.3.01:jar -DremoteRepositories=https://oss.sonatype.org/content/repositories/snapshots -Ddest=interfaces.jar

     Then install them in your enclosed environment.

     run:
     mvn org.apache.maven.plugins:maven-install-plugin:3.0.0-M1:install-file  -Dfile=./interfaces.jar -DgroupId=io.hkube -DartifactId=interfaces -Dversion=1.3.01 -Dpackaging=jar
     mvn org.apache.maven.plugins:maven-install-plugin:3.0.0-M1:install-file  -Dfile=./java-algo-parent.xml -DgroupId=io.hkube -DartifactId=java-algo-parent -Dversion=1.3.01 -Dpackaging=pom

   c) pom.xml.
        * Create a java maven project, have the projects pom.xml inherit from io.hkube:java-algo-parent:1.3.01:pom
             <parent>
...
3. Run algorithm locally (for debugging) and as part of a pipeline on the cluster.
    ...
    a. Main class.
       Java main class is hkube.algo.wrapper.Main from artifact io.hkube:wrapper:1.3.01 (also found in https://oss.sonatype.org/content/repositories/snapshots)
    
    b. Runtime ClassPath
    
       1) io.hkube:wrapper:1.3.01 and its dependencies.
